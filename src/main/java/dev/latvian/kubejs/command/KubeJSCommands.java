...
public class KubeJSCommands {
	...
	private static int itemInfo(ServerPlayerEntity player) {
		...
		List<Identifier> tags = new ArrayList<>(getTagsFor(player.world.getTagManager().getItems(), stack.getItem()));
		tags.sort(null);
		...
		for (Identifier id : tags) {
			...
			c.styled(style -> style.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new LiteralText("Click to copy"))));
	...
	public static <T> Collection<Identifier> getTagsFor(TagGroup<T> container, T object) {
		List<Identifier> list = Lists.newArrayList();
		
		for (Map.Entry<Identifier, Tag<T>> identifierTagEntry : container.getTags().entrySet()) {
